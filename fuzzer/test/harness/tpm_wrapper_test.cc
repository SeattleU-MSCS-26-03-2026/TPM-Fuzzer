...
extern "C" {
    ...
    void _plat__Signal_Reset(void) {
        MockTPM::tracker.signal_reset_called = true;
    }
    
    void _plat__SetNvAvail(void) {
        MockTPM::tracker.set_nv_avail_called = true;
    }
    ...
    void _TPM_Hash_End(void) {
        MockTPM::tracker.hash_end_called = true;
    }
    
    void _plat__NvCommit(void) {
        MockTPM::tracker.nv_commit_called = true;
    }
...
TEST_CASE("TPMManufactureIfNeeded - TPM already manufactured", "[tpm_wrapper][manufacture]") {
...
TEST_CASE("TPMManufactureIfNeeded - TPM needs manufacturing and succeeds", "[tpm_wrapper][manufacture]") {
    MockTPM::tracker.reset();
...
TEST_CASE("TPMManufactureIfNeeded - TPM manufacturing fails", "[tpm_wrapper][manufacture]") {
    MockTPM::tracker.reset();
    MockTPM::tracker.nv_needs_manufacture_result = true;
...
TEST_CASE("TPMStartup - calls all required platform functions", "[tpm_wrapper][startup]") {
    MockTPM::tracker.reset();
...
TEST_CASE("TPMStartup - functions called in correct order", "[tpm_wrapper][startup]") {
    MockTPM::tracker.reset();
...
TEST_CASE("TPMShutdown - calls all cleanup functions", "[tpm_wrapper][shutdown]") {
    MockTPM::tracker.reset();
...
TEST_CASE("TPMShutdown - cleanup functions called in correct order", "[tpm_wrapper][shutdown]") {
    MockTPM::tracker.reset();
...
TEST_CASE("Full lifecycle - manufacture, startup, command, shutdown", "[tpm_wrapper][integration]") {
    MockTPM::tracker.reset();
